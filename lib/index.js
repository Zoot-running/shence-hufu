...
var DEFAULT_RESOURCE_CLASS = "default";
function resourceClassOf(item) {
...
var HufuCampaign = class _HufuCampaign {
  ...
  /** 落账路径条目（全局解题图）。 */
  recordKnowledge(itemId, entries) {
   ...
   */
  async interruptItem(itemId) {
    const view = this.ledger.view(itemId);
    if (view === void 0 || !isActive(view.state)) return;
...
var ResourceQueue = class {
   ...
   */
  acquire(holderId, opts) {
    const existing = this.pending.get(holderId);
    if (existing !== void 0) return existing;
    const promise = new Promise((resolve) => {
      const waiter = { holderId, enqueuedAt: Date.now(), position: this.queue.length + 1, resolve };
    ...
    void promise.finally(() => {
      if (this.pending.get(holderId) === promise) this.pending.delete(holderId);
    });
    return promise;
  }
  ...
  evict(holderId, reason) {
    let removed = false;
    for (let i = this.queue.length - 1; i >= 0; i--) {
      const w = this.queue[i];
      if (w.holderId !== holderId) continue;
      if (w.timer !== void 0) clearTimeout(w.timer);
  ...
  startPump() {
    if (this.pumpTimer !== void 0) return;
  ...
  stopPumpIfIdle() {
    if (this.queue.length === 0 && this.pumpTimer !== void 0) {
  ...
  async pump() {
    ...
    try {
      ...
      this.granting = true;
      try {
        if (this.cfg.grant !== void 0) await this.cfg.grant(waiter.holderId);
        this.granted += 1;
...
function createHostPorts(ctx, agent, holder, subagentProvider, jisi, boardNamespace) {
  ...
  const dispatch = {
    async dispatch(item, _seed) {
      const jisiNow = jisi ?? ctx.get?.("jisi");
      ...
      if (jisiNow !== void 0) {
        if (item.continuable === true) {
          const result2 = jisiNow.delegate(agent, work, { ...opts, background: true });
        ...
        void result.report.then((report) => {
          ...
          for (const listener of settleListeners) {
            try {
              listener({ itemId: item.id, status: report.status, text: report.text });
            } catch {
            }
          }
        });
  ...
  const board = {
    pathOf(group) {
      ...
      const ns = boardNamespace !== void 0 && boardNamespace !== "" ? boardNamespace.replace(/[^A-Za-z0-9._-]/g, "_") : void 0;
      const dir = ns !== void 0 ? join(process.cwd(), "boards", ns, safe) : join(process.cwd(), "boards", safe);
...
function createHufuService(ctx, subagentProvider) {
  ...
  const settleListenersByCampaign = /* @__PURE__ */ new Map();
  const require2 = (id) => {
  ...
  return {
    ...
    recordKnowledge(id, itemId, entries) {
      require2(id).recordKnowledge(itemId, entries);
      persist(id, require2(id));
    },
    knowledgeOf(id, itemId) {
      return require2(id).knowledgeOf(itemId);
    },
    ...
    resourceQueue: (config) => new ResourceQueue(config),
    finish(id) {
      const campaign = require2(id);
      persist(id, campaign);
      try {
...
var inject = ["subagents", "tools", "jisi"];
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const { id } = service.createCampaign(agent, {
        ...
        ...args.boardNamespace !== void 0 ? { boardNamespace: args.boardNamespace } : {}
      });
